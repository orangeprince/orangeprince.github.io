...
\large
\begin{aligned}
\underset{w}{\operatorname{argmin}}  \quad \Omega(\phi(w))  + C \sum\_{i=1}^l \ell(y_i, \phi(w)^T\phi(x_i))
\end{aligned}
$$
...
\large
\begin{aligned}
\underset{w}{\operatorname{argmin}}  \quad \parallel w \parallel_2^2  + C \sum\_{i=1}^l \ell(y_i, w^Tx_i)
\end{aligned}
$$
