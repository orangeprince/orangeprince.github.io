...
最后，LIBSVM也支持基于SVM的回归模型，即SVR。与分类模型类似，SVR也分为$C$-SVR和$\nu$-SVR。SVR的目标函数与SVM的分类模型稍有区别。由于回归问题预测值与目标值的偏差可大可小，因此SVR使用了两个slack variable用来刻画预测的误差边界。虽然存在一定的差别，但是SVR的基本思路和优化算法与SVM分类模型还是基本一致的。

在LIBSVM的实现中，上面五种模型，既$C$-SVM，$\nu$-SVM，One-class SVM，$C$-SVR，$/nu$-SVR，最终都可以转化为一个更通用的优化目标，然后用同样的策略进行求解，这也是LIBSVM所实现的主要功能。在实际使用中，最常用到的方法还是$C$-SVM，这也是最传统的SVM分类模型。
